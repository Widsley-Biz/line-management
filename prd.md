...
| ID | ページ | 現象 | 修正方針 |
|----|--------|------|----------|
| BUG-01 | `/mobile/devices` | ＜＞ナビで月を変えても右上ピッカーの表示が更新されない | `MonthPicker` の `defaultValue` → `value` に変更 |
| BUG-02 | `/tenants/new` | 登録後に取引先詳細へ遷移する（一覧に戻るべき） | `redirect()` の遷移先を `/tenants` に変更 |

#### 6.4.1 CSV課金項目マスタ管理（インポート改善）
...
| 2 | CSVインポート時に各項目を自動判別してDBに取り込む |
| 3 | 未登録の新規項目が検出された場合、インポート前にポップアップを表示し「課金」「非課金」を選択してからDBに登録する |
| 4 | 各項目に「今後も継続して取り込む」「今回のみ取り込む」の設定を持たせる |
| 5 | 項目設定完了後、CSVそのものを取り込んで通常の請求額登録フローを実行する |

**新規テーブル: `mobile_billing_items`（課金項目マスタ）**

```
mobile_billing_items
  id           -- UUID
  item_name    -- SoftBank CSV上の項目名（ユニーク）
  is_billable  -- true: 課金 / false: 非課金
  is_recurring -- true: 継続取込 / false: 今回のみ
  created_at
  updated_at
```
