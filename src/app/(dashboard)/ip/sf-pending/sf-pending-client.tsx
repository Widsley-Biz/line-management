...
export function SfPendingClient({ rows }: { rows: PendingRow[] }) {
  ...
  async function handleExclude() {
    setExcluding(true);
    try {
      await fetch("/api/ip/billing-status", {
        method: "PATCH",
      ...
      {rows.length === 0 ? (
        ...
        <>
          ...
          {months.map((month) => {
            ...
            const monthTotal = monthRows.reduce((s, r) => s + r.totalAmount, 0);
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b bg-white">
                        ...
                        <th className="text-right px-4 py-2 font-medium text-gray-500 text-xs">固定分</th>
                        <th className="text-right px-4 py-2 font-medium text-gray-500 text-xs">携帯＋ナビ分</th>
                        <th className="text-right px-4 py-2 font-medium text-gray-500 text-xs">合計</th>
                    ...
                    <tbody>
                      {monthRows.map((r) => (
                        <tr key={r.id} className={`border-b hover:bg-gray-50 ${checked.has(r.id) ? "bg-blue-50" : ""}`}>
                          ...
                          <td className="px-4 py-3 text-right text-gray-600">{formatYen(r.fixedAmount)}</td>
                          <td className="px-4 py-3 text-right text-gray-600">{formatYen(r.mobileNaviAmount)}</td>
                          ...
                          <td className="px-4 py-3 text-right">
                            ...
                            ) : r.sfOpportunityId && r.totalAmount > 0 ? (
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => handleSend(r.id)}
