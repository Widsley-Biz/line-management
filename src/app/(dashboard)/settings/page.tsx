                    ...
                    <td className="px-3 py-2">
                      <div className="flex gap-1">
                        {u.id === currentSession?.user?.id && !u.sfRefreshToken && (
                          // OAuthリダイレクトのためSPA遷移ではなくフルページ遷移が必要
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between py-2 border-b">
              <span className="text-gray-600">SF_PRICEBOOK2_ID</span>
              <span className="font-mono text-xs">{process.env.SF_PRICEBOOK2_ID ? "設定済み" : "未設定"}</span>
            </div>
            <div className="flex justify-between py-2 border-b">
              <span className="text-gray-600">SF_PRICEBOOK_ENTRY_ID_CC01（IP回線通話料_国内固定番号宛）</span>
              <span className="font-mono text-xs">{process.env.SF_PRICEBOOK_ENTRY_ID_CC01 ? "設定済み" : "未設定"}</span>
            </div>
            <div className="flex justify-between py-2 border-b">
              <span className="text-gray-600">SF_PRICEBOOK_ENTRY_ID_CC02（IP回線通話料_国内携帯番号宛）</span>
              <span className="font-mono text-xs">{process.env.SF_PRICEBOOK_ENTRY_ID_CC02 ? "設定済み" : "未設定"}</span>
            </div>
            <div className="flex justify-between py-2">
              <span className="text-gray-600">SF_PRICEBOOK_ENTRY_ID_MOBILE（携帯回線超過分）</span>
              <span className="font-mono text-xs">{process.env.SF_PRICEBOOK_ENTRY_ID_MOBILE ? "設定済み" : "未設定"}</span>
            </div>
          </div>
